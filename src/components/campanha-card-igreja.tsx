...
import {
  ...
  alternarEncerramentoCampanhaAction,
  removerCampanhaAction,
} from "@/app/igreja/campanhas/actions";

export function CampanhaCardIgreja({
  campanha,
  arrecadado,
}: {
  campanha: Campanha;
  ...
  if (editando) {
    return (
      <Card>
        <form
          action={atualizarCampanhaAction}
        ...
        >
          ...
          <label className="flex flex-col gap-1">
            ...
            <input
              name="imagemEmoji"
              defaultValue={campanha.imagemEmoji}
              className="rounded-xl border border-border px-4 py-3"
            />
          </label>
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm text-muted">
              Prazo: {formatarData(campanha.prazo)}
            </p>
          </div>
        </div>
        {campanha.encerrada ? (
          ...
          <Badge tone="success">Em captação</Badge>
        )}
      </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <form action={alternarEncerramentoCampanhaAction}>
            ...
            <input
              type="hidden"
              name="encerrada"
              value={(!campanha.encerrada).toString()}
            />
