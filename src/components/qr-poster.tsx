  ...
  return (
    <div>
      <div className="mb-6 flex flex-col gap-3 print:hidden sm:flex-row sm:items-center sm:justify-between">
        <Link href={voltarHref} className="flex items-center gap-1 text-sm font-bold text-muted hover:text-primary">
          <ArrowLeft size={16} />
          Voltar
        </Link>
        <div className="flex flex-col gap-2 sm:flex-row">
          <a href={qrDataUrl} download={`qrcode-${titulo.toLowerCase().replace(/\s+/g, "-")}.png`}>
      ...
      <Card className="mx-auto max-w-md text-center print:border-0 print:shadow-none">
        ...
        <img src={qrDataUrl} alt={`QR Code — ${titulo}`} className="mx-auto mb-4 h-64 w-64" />

        <h1 className="text-xl font-bold">{titulo}</h1>
        <p className="mt-1 text-muted">{subtitulo}</p>
        <p className="mt-4 text-xs text-muted">Aponte a câmera do celular para contribuir</p>
