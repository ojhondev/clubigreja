...
export function FormLoginIgreja() {
  const [estado, action, pending] = useActionState(entrarIgrejaEmailAction, ESTADO_INICIAL);

  return (
    <form action={action} className="space-y-4">
      <label className="flex flex-col gap-1">
        <span className="text-sm font-bold text-muted">E-mail</span>
        <input name="email" type="email" required placeholder="voce@suaigreja.org.br" className="rounded-xl border border-border px-4 py-3" />
      </label>
      <label className="flex flex-col gap-1">
