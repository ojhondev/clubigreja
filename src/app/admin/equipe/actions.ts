...
export async function convidarWebmasterAction(
  ...
  if (!primario)
    return { erro: "Apenas o Master Primário pode convidar novos membros." };

  const nome = String(formData.get("nome") ?? "").trim();
  const email = String(formData.get("email") ?? "").trim();
  const podeGerenciarPagamentos =
    formData.get("podeGerenciarPagamentos") === "on";
  ...
  if (membros.some((m) => m.email.toLowerCase() === email.toLowerCase())) {
    return {
      erro: "Já existe um membro (ou convite pendente) com esse e-mail.",
    };
  }

  await gerarConviteWebmaster({
    nome,
    email,
    podeGerenciarPagamentos,
    podeAprovarIgrejas,
    convidadoPorId: primario.id,
  });
  revalidatePath("/admin/equipe");
  return { sucesso: true };
}

export async function atualizarPermissoesWebmasterAction(
  formData: FormData,
): Promise<void> {
  const primario = await exigirMasterPrimario();
  if (!primario) return;
  ...
  await atualizarPermissoesWebmaster(id, {
    podeGerenciarPagamentos,
    podeAprovarIgrejas,
  });
  revalidatePath("/admin/equipe");
