...
export default async function FieisAdminPage() {
  const [fieis, igrejas] = await Promise.all([
    getTodosFieis(),
    getTodasIgrejas(),
  ]);
  ...
  return (
    <div>
      <PageHeader
        title="Fiéis na plataforma"
        subtitle="Acesse a conta de qualquer fiel para conferir telas e testar fluxos."
      />
      ...
      <div className="space-y-3">
          ...
          >
            <div>
              ...
              <p className="text-sm text-muted">
                {fiel.telefone} ·{" "}
                {igrejaPorId.get(fiel.igrejaId)?.nome ??
        ...
        {fieis.length === 0 && (
          <p className="text-muted">Nenhum fiel cadastrado ainda.</p>
        )}
      </div>
    </div>
  );
