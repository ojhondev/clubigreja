...
import { criarWebmasterPrimarioAction, entrarWebmasterAction, type EstadoWebmaster } from "./actions";

const ESTADO_INICIAL: EstadoWebmaster = {};

export function FormWebmaster({ modo }: { modo: "login" | "setup" }) {
  const action = modo === "setup" ? criarWebmasterPrimarioAction : entrarWebmasterAction;
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <label className="flex flex-col gap-1">
        ...
        <input name="email" type="email" required placeholder="voce@dizipay.com" className="rounded-xl border border-border px-4 py-3" />
      </label>
      <label className="flex flex-col gap-1">
      ...
      <Button type="submit" disabled={pending} className="w-full">
        {pending ? "Enviando…" : modo === "setup" ? "Criar Master Primário" : "Entrar"}
      </Button>
    </form>
  );
}
